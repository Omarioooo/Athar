...
import {
    ...
    searchCampaignsService,
} from "../../services/campaignService";

export default function Campaigns() {
    const [refFirstcampaign, inViewFirst] = useInView({
        triggerOnce: true,
        threshold: 0.2,
    });
    ...
    const loadCampaigns = async (page) => {
        ...
        setErrorMsg("");
        try {
            const data = await fetchAllCampaigns(page, pageSize); // جاهزة مع daysLeft & progress
    ...
    const handleSearch = async () => {
        ...
        setErrorMsg("");
        try {
            const data = await searchCampaignsService(keyword); // جاهزة مع daysLeft & progress
            setCampaigns(data);
        } catch (error) {
            setCampaigns([]);
            setErrorMsg(error.message);
    ...
    const handleCategoryClick = async (cat) => {
        ...
        setErrorMsg("");
        try {
            const data = await fetchCampaignsByType(cat.id);
            setCampaigns(data);
        } catch (error) {
            setCampaigns([]);
            setErrorMsg(error.message);
        ...
        >
            <div className="container mt-3 campaign-section">
                ...
                <div className="serch-company">
                    ...
                    />
                    <button onClick={handleSearch} className="btn-compaign">
                        بحث
                    </button>
                </div>
                    ...
                    <>
                        ...
                        <div className="row">
                            {campaigns.map((cmp) => (
                                <div
                                    key={cmp.id}
                                    className="col-xl-4 col-lg-6 col-md-12 col-sm-12 mb-4"
                                >
                                    <div
                                    ...
                                    >
                                        ...
                                        <div className="card-body">
                                            <h4 className="card-title cmp-title">
                                                {cmp.title}
                                            </h4>
                                            <p className="card-text campaign-description">
                                                {cmp.description}
                                            </p>
