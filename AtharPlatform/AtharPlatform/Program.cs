...
// Add IsOpen column if missing
try
{
    using var scope = app.Services.CreateScope();
    var db = scope.ServiceProvider.GetRequiredService<Context>();
    Console.WriteLine("[Startup] Adding IsOpen columns if missing...");
    
    // Add IsOpen to CharityVolunteers table
    try
    {
        db.Database.ExecuteSqlRaw(@"
            ALTER TABLE ""CharityVolunteers"" 
            ADD COLUMN IF NOT EXISTS ""IsOpen"" boolean NOT NULL DEFAULT true;
        ");
        Console.WriteLine("[Startup] IsOpen column added/verified for CharityVolunteers");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[Startup] CharityVolunteers.IsOpen already exists or error: {ex.Message}");
    }
    
    // Add IsOpen to CharityVendorOffers table
    try
    {
        db.Database.ExecuteSqlRaw(@"
            ALTER TABLE ""CharityVendorOffers"" 
            ADD COLUMN IF NOT EXISTS ""IsOpen"" boolean NOT NULL DEFAULT true;
        ");
        Console.WriteLine("[Startup] IsOpen column added/verified for CharityVendorOffers");
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[Startup] CharityVendorOffers.IsOpen already exists or error: {ex.Message}");
    }
}
catch (Exception isOpenEx)
{
    Console.WriteLine($"[Startup] Failed to add IsOpen columns: {isOpenEx.Message}");
}
...
try
{
    using var scope = app.Services.CreateScope();
    var db = scope.ServiceProvider.GetRequiredService<Context>();
    Console.WriteLine("[Startup] Fixing charity ImageUrls...");
    
    // Fix charity 107
    var charity107 = db.Charities.FirstOrDefault(c => c.Id == 107);
    if (charity107 != null)
    {
        Console.WriteLine($"[Startup] Charity 107 current ImageUrl: '{charity107.ImageUrl ?? "NULL"}'");
        db.Database.ExecuteSqlRaw(@"
            UPDATE ""Charities"" 
            SET ""ImageUrl"" = '/uploads/charities/charity_107_20251124_233717.jpg'
            WHERE ""Id"" = 107;
        ");
        Console.WriteLine("[Startup] Charity 107 ImageUrl fixed");
    }
    
    // Fix charity 113 (wrong port 7032 -> 5192)
    var charity113 = db.Charities.FirstOrDefault(c => c.Id == 113);
    if (charity113 != null)
    {
        Console.WriteLine($"[Startup] Charity 113 current ImageUrl: '{charity113.ImageUrl ?? "NULL"}'");
        db.Database.ExecuteSqlRaw(@"
            UPDATE ""Charities"" 
            SET ""ImageUrl"" = '/uploads/charities/cb17a6d2-bb41-434f-8ed2-c74c21b279a0_GptImage.png'
            WHERE ""Id"" = 113;
        ");
        Console.WriteLine("[Startup] Charity 113 ImageUrl fixed (removed wrong port)");
    }
}
